 ...
 * - When `stage === 'ux'` and the requested taskType is `architecture`, the
 *   route is deferred (no docs returned) to prevent backtracking into
 *   architecture decisions while UX is still being explored.
 *
...
/**
 * §9.4: refuse to load architecture documents while UX is still being decided.
 * The question/task is recorded as deferred so it can be re-raised in the
 * architecture phase.
 */
function shouldDefer(taskType, stage) {
  if (taskType === 'architecture' && stage === 'ux') {
    return {
      deferred: true,
      reason:
        'Architecture documents are not loaded during UX phase. The question will be deferred until the architecture phase.',
    };
  }
