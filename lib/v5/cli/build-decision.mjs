 ...
 *     process.cwd() looking for `control/v5/`.
 *   - Reads {controlRoot}/features/{slug}/decisions.json via readDecisions.
 *   - Searches phases.ux/ui/architecture for a decision whose `id` matches.
 *   - Calls buildVisualFragment(decision) with default options.
 ...
 * against `cwd`. Otherwise, walks up from `cwd` looking for `control/v5/`.
 * Throws if not found.
 */
async function resolveControlRoot({ controlRoot, cwd }) {
  ...
  const found = await findNearestControlV5(startDir);
  if (!found) {
    throw new Error(
...
export async function buildDecisionFragment({ slug, decisionId, controlRoot, cwd } = {}) {
  ...
  const decisionsPath = path.join(resolvedControlRoot, 'features', slug, 'decisions.json');
  try {
  ...
  const fragment = buildVisualFragment(decision);

  const outDir = path.join(resolvedControlRoot, 'features', slug, 'decisions');
