...
export async function buildPacket({ task, route, stage, slug, controlRoot = null } = {}) {
  ...
  // Deferred routes return early — no docs to load, 0 tokens. The route's
  // usageNote still propagates into instructions[] so the orchestrator sees
  // the visual-fragment contract even when the dispatch is refused.
  if (route.deferred) {
    return {
...
/**
 * Build the `instructions[]` array surfaced on every dispatch packet. Pulls
 * each route's `usageNote` into a flat string list so the orchestrator and
 * subagent can read the hard rules at a glance.
 */
