 ...
 *
 * File layout (paths are relative to the project root, the directory
 * containing `control/v5/`):
 * - Decisions: {controlRoot}/control/v5/features/{slug}/decisions.json
 * - Fragments: {controlRoot}/control/v5/features/{slug}/decisions/{decision-id}.html
 */
...
export async function lintDecisions({ slug, controlRoot }) {
  ...
  const decisionsDir = path.join(controlRoot, 'control', 'v5', 'features', slug, 'decisions');
  let fragmentFiles = [];
  try {
...
export async function lintAllFeatures({ controlRoot }) {
  ...
  const featuresDir = path.join(controlRoot, 'control', 'v5', 'features');
  let featureDirs = [];
  try {
...
async function main() {
  ...
  // Resolve controlRoot if not provided — walk up from cwd looking for the
