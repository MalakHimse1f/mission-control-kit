...
/**
 * Build a sort comparator that orders features by their position in the
 * project's `state.json` `features[]` array (the canonical build order).
 * Features missing from state.json fall after listed ones, sorted alphabetically.
 *
 * @param {object|null} state — parsed control/v5/state.json, or null
 */
export function buildOrderComparator(state) {
  const slugs = Array.isArray(state?.features)
    ? state.features.map((f) => f?.slug).filter((s) => typeof s === 'string')
    : [];
...
export async function loadDashboardData({ controlRoot } = {}) {
  ...
  const allItems = [...features].sort(buildOrderComparator(state));
