 ...
 *
 * Reads from disk:
 *   - {controlRoot}/state.json          (optional, currently unused but reserved)
 *   - {controlRoot}/features/<slug>/status.json
 *   - {controlRoot}/features/<slug>/decisions.json (optional)
 *
...
export async function loadDashboardData({ controlRoot } = {}) {
  ...
  const featuresRoot = path.join(controlRootAbs, 'features');
  const statePath = path.join(controlRootAbs, 'state.json');
