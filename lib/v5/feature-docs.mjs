 ...
 *
 * Directory layout (all under `{controlRoot}/control/v5/features/{slug}/`,
 * where `controlRoot` is the project root that CONTAINS `control/v5/`):
 *   - spec.md
 ...
 *   - `controlRoot` defaults to the nearest project root (directory
 *     containing `control/v5/`) walking up from `process.cwd()`, matching
 *     the convention used in decisions.mjs.
 ...
 * directory whose child is `control/v5/`. Returns that directory (the
 * project root), NOT `control/v5/` itself.
 */
...
function featureDir(controlRoot, slug) {
  return path.join(controlRoot, 'control', 'v5', 'features', slug);
}
