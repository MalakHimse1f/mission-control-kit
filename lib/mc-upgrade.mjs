...
export function installStampPath(controlRoot) {
  return path.join(controlRoot, '.mc', 'install.json');
}

export function readInstallStamp(controlRoot) {
  const p = installStampPath(controlRoot);
  if (!fs.existsSync(p)) return null;
  return JSON.parse(fs.readFileSync(p, 'utf8'));
}

export function writeInstallStamp(controlRoot, stamp) {
  const p = installStampPath(controlRoot);
...
export async function runMigrations({ kitRoot, controlRoot, fromVersion, toVersion, dryRun = false }) {
  ...
  for (const file of files) {
    ...
    if (!dryRun && typeof mod.up === 'function') {
      await mod.up({ controlRoot, kitRoot });
    }
...
export function resolvePaths(projectRoot, kitRoot) {
  const controlRoot = path.join(projectRoot, 'docs/superpowers/control');
  const kitControl = path.join(kitRoot, 'control');
  return { controlRoot, kitControl };
}

export function resolveLocalKitRoot(projectRoot, kitFolder = 'mission-control-kit-v4') {
  const stamp = readInstallStamp(path.join(projectRoot, 'docs/superpowers/control'));
  const folder = stamp?.kitPath ?? kitFolder;
...
export async function resolveKitRoot({
  ...
  const local = resolveLocalKitRoot(projectRoot, manifest?.kitFolder ?? 'mission-control-kit-v4');
...
export async function runUpgrade({
  ...
  const { controlRoot, kitControl } = resolvePaths(projectRoot, kitRoot);
  ...
  let backupDir = null;
  if (!dryRun && fromVersion) {
    const ts = new Date().toISOString().replace(/[:.]/g, '-');
    backupDir = path.join(controlRoot, '.mc/backups', ts);
    backupPreservedFiles({ projectControl: controlRoot, backupDir });
  }

  const { copied, skipped } = syncControlPlane({ kitControl, projectControl: controlRoot, dryRun });
  ...
  if (!dryRun) {
    ...
    const report = upgradeReport({
      fromVersion,
      toVersion,
      copiedControl: copied,
      skippedControl: skipped,
      migrations,
      backupDir,
      dryRun: false,
    });
    fs.writeFileSync(path.join(controlRoot, '.mc/UPGRADE-REPORT.md'), report);
  }
